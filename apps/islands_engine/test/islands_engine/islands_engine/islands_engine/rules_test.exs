defmodule IslandsEngine.RulesTest do
  ...
  describe ":players_set" do
    ...
    test "can position islands without changing state", %{rules: rules} do
      {:ok, %Rules{state: :players_set} = rules} =
        Rules.check(rules, {:position_islands, :player_one})
  ...
  describe ":player_one_turn" do
    ...
    test "player_one can guess a coordinate", %{rules: rules} do
      {:ok, %Rules{state: :player_two_turn}} =
        Rules.check(rules, {:guess_coordinate, :player_one})
    end
    ...
    test "player_one can win the game", %{rules: rules} do
      {:ok, %Rules{state: :game_over}} = Rules.check(rules, {:win_check, :win})
    end

    test "state is unchanged if player_one does not win", %{rules: rules} do
  ...
  describe ":player_two_turn" do
    ...
    test "player_two can guess a coordinate", %{rules: rules} do
      {:ok, %Rules{state: :player_one_turn}} =
        Rules.check(rules, {:guess_coordinate, :player_two})
    end
    ...
    test "player_two can win the game", %{rules: rules} do
      {:ok, %Rules{state: :game_over}} = Rules.check(rules, {:win_check, :win})
    end

    test "state is unchanged if player_two does not win", %{rules: rules} do
      {:ok, ^rules} = Rules.check(rules, {:win_check, :no_win})
    end
  end
end
